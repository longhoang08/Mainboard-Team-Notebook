...
Đường tròn đi qua 3 điểm cho trước
	Let 𝐴 = (0, 0) centers are 
	(𝐶𝑦(𝐵𝑥^2 + 𝐵𝑦^2) − 𝐵𝑦(𝐶𝑥^2 + 𝐶𝑦^2))/𝐷 
		and
	(𝐵𝑥(𝐶𝑥^2 + 𝐶𝑦^2) − 𝐶𝑥(𝐵𝑥^2 + 𝐵𝑦^2))/𝐷 
		where 
	𝐷 = 2(𝐵𝑥𝐶𝑦 − 𝐵𝑦𝐶𝑥).
...
Pick’s theorem (Đếm số điểm nguyên trong đa giác có các đỉnh nguyên) 
	𝐼 = 𝐴 − 𝐵/2 + 1
	where 
		𝐴 is the area of a lattice polygon,
		𝐼 is number of lattice points inside it,
		𝐵 is number of lattice points on the boundary.
		Number of lattice points minus one on a line segment from (0, 0) and (𝑥, 𝑦) is gcd(𝑥, 𝑦).
 
Tích chập 3D: 𝑎 × 𝑏 = (𝑎𝑦𝑏𝑧 − 𝑎𝑧𝑏𝑦, 𝑎𝑧𝑏𝑥 − 𝑎𝑥𝑏𝑧, 𝑎𝑥𝑏𝑦 − 𝑎𝑦𝑏𝑥)
 
Distance from line 𝐴𝐵 to 𝑃 (for any dimension) : |(A-P) x (B-P)| / |A-B|
